...
/*   Updated: 2024/07/12 13:21:17 by trimize          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	freeforks(t_threads *th, t_philo *philo)
{
	...
	while (i < philo->num_philo)
		(pthread_mutex_destroy(th->forks[i]), free(th->forks[i++]));
	free(th->forks);
}
...
{
	freeforks(th, philo);
	pthread_mutex_destroy(th->stop);
	free(th->stop);
	pthread_mutex_destroy(th->eat);
	free(th->eat);
	pthread_mutex_destroy(th->print);
	free(th->print);
	free(th->has_to_stop);
	free(philo);
}
